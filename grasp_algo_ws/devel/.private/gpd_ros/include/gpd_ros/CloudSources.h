...
struct Printer< ::gpd_ros::CloudSources_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::gpd_ros::CloudSources_<ContainerAllocator>& v)
  {
    ...
    if (v.camera_source.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.camera_source.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::Int64_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.camera_source[i]);
    }
    if (v.camera_source.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "view_points: ";
    if (v.view_points.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.view_points.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.view_points[i]);
    }
    if (v.view_points.empty() || false)
      s << "]";
  }
